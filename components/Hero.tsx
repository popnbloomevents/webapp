...
export function Hero(){
  ...
  useEffect(()=>{const node=hero.current;if(!node||matchMedia("(prefers-reduced-motion: reduce)").matches)return;let frame=0;const update=()=>{frame=0;const y=Math.min(Math.max(window.scrollY,0),900);const mobile=matchMedia("(max-width: 640px)").matches;const factor=(desktop:number,compact:number)=>y*(mobile?compact:desktop);node.style.setProperty("--hero-shift",`${factor(.34,.2)}px`);node.style.setProperty("--hero-eyebrow-shift",`${factor(.07,.035)}px`);node.style.setProperty("--hero-title-shift",`${factor(.12,.06)}px`);node.style.setProperty("--hero-body-shift",`${factor(.17,.085)}px`);node.style.setProperty("--hero-actions-shift",`${factor(.22,.11)}px`);node.style.setProperty("--hero-card-shift",`${factor(-.25,-.12)}px`);node.style.setProperty("--hero-note-shift",`${factor(.2,.08)}px`);node.style.setProperty("--hero-strip-shift",`${factor(.11,.055)}px`);node.style.setProperty("--hero-orbit-shift",`${factor(.26,.12)}px`);node.style.setProperty("--hero-orbit-two-shift",`${factor(-.19,-.09)}px`)};const onScroll=()=>{if(!frame)frame=requestAnimationFrame(update)};update();addEventListener("scroll",onScroll,{passive:true});return()=>{removeEventListener("scroll",onScroll);if(frame)cancelAnimationFrame(frame)}} ,[]);
