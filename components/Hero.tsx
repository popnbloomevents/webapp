...
export function Hero(){
  ...
  useEffect(()=>{const node=hero.current;if(!node||matchMedia("(prefers-reduced-motion: reduce)").matches)return;let frame=0;const update=()=>{frame=0;const y=Math.min(window.scrollY,850);node.style.setProperty("--hero-shift",`${y*.11}px`);node.style.setProperty("--hero-card-shift",`${y*-.045}px`)};const onScroll=()=>{if(!frame)frame=requestAnimationFrame(update)};update();addEventListener("scroll",onScroll,{passive:true});return()=>{removeEventListener("scroll",onScroll);if(frame)cancelAnimationFrame(frame)}} ,[]);
